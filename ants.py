...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if self.ant.container and self.ant.ant != None: #in case only bodyguard/container ant is removed
                self.ant = self.ant.ant #if self.ant.ant else None
            elif isinstance(self.ant, QueenAnt) and self.ant.true_queen: 
                return      #we can't remove the first/true queen
            else:
                self.ant = None
...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        return self.container and self.ant == None and ant.container == False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        current_place = self.place #this will keep track of where we are
        bee_not_found = True 
        distance = 0
        while bee_not_found: #need the loop to stop once we've found a bee
            #checking that the place is within range and then pulling a bee out of current_place if it exists
            if distance >= self.min_range and distance < self.max_range and len(current_place.bees) > 0: 
                return random_or_none(current_place.bees)
                bee_not_found = False
            else:
                if current_place.entrance != None and type(current_place.entrance) != Hive: #confirming not Hive
                    current_place = current_place.entrance  #moving the current_place over by 1
                    distance += 1
                elif type(current_place.entrance) == Hive: #situation where we've reached the Hive (no bees in range)
                    return None
                    bee_not_found = False
                # else: 
                    # return None               #this comment block may be superfluous
                    # bee_not_found = False
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        if amount >= self.armor: #does damage on death
            for bee in self.place.bees[:]:
                bee.reduce_armor(self.damage)
        Ant.reduce_armor(self, amount) #parent function
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        self.QPcolony_queen = colony_queen
        self.QPant_queen = ant_queen

    @property
    def bees(self):
        return self.QPcolony_queen.bees + self.QPant_queen.bees
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        if QueenAnt.num_queens == 0:
            ...
            self.doubled_ants = set()   #set is an unordered list with no duplicates
    ...
    def action(self, colony):
        ...
        if not self.true_queen:             #kill "imposter queens"
            self.reduce_armor(self.armor)
            return                              #to get out of the loop
        colony.queen = QueenPlace(colony.queen, self.place)
        
        #double damage of bees in the same tunnel
        same_tunnel_ants = self.get_tunnel_ants()
        for ant in (same_tunnel_ants - self.doubled_ants): #avoiding duplicates
            ant.damage *= 2
        self.doubled_ants = self.doubled_ants.union(same_tunnel_ants) #keeping track of which ants have doubled dmg already
        
        ScubaThrower.action(self, colony) #throwing leaf/scuba ant action
        
    def get_tunnel_ants(self):
        ants = set()
        if self.place.ant is not self:   #if Queen has a bodyguard, we need to add it
            ants.add(self.place.ant)
            
        #searching toward original queen
